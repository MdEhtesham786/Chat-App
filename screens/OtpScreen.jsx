...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, ActivityIndicator, SafeAreaView, KeyboardAvoidingView, TouchableWithoutFeedback, Keyboard } from "react-native";
...
export default OtpScreen = ({ navigation, route }) => {
    ...
    const { hashedEmail, userID } = route.params;
    ...
    const handleVerifyButton = async () => {
        try {
            if (otp) {
                ...
                setIsLoading(true);
                setOTP([]);
                let res = await axios.post('/auth/verify-otp', { data: otp, userID });
                const { data } = res;
                if (data.success) {
